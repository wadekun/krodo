# Coda — Coding Agent 总体架构与技术选型设计

> 项目代号：**Coda** ｜ 定位：local-first coding agent CLI ｜ 版本：v0.4
>
...
- **Workspace（工作区/执行上下文，cross-cutting）**：项目根的发现与校验、`.coda/`/`AGENTS.md`/`.codaignore`/`git_root` 的查找起点、所有 side-effect 类工具的路径围栏基准。是 Sandbox、Memory、Git checkpoint 的"单一事实源"。一个 Coda 进程只构造一次，启动后不可变。
...
- **Python（主推）**
  ...
  - 分发：`pipx install coda` + `uv tool install coda`，可接受；后期可用 PyInstaller / shiv 打包单文件。
...
可观测性：
- 日志：`structlog`，输出 JSONL 到 `~/.local/state/coda/logs/`。
...
打包分发：
- MVP：`pyproject.toml` + `uv` / `pipx install coda`。
...
Phase 0 的 `prototype.py` 暴露了一个真实 bug：`ROOT = Path(__file__).resolve().parent.parent` 导致用户在 `/tmp/coda-sandbox` 启动时仍然写到了 coda 项目目录，路径围栏同时失效。根因是"项目根"没有被当成一等概念显式传递，而是由脚本自行推断。

`Workspace` 是整个 Coda 进程的"单一事实源"——**一个进程只构造一次，启动后不可变**。所有 side-effect 类工具的路径基准、配置、Git 根、Memory 路径均由它提供，**任何子模块不允许自行调用 `Path.cwd()` 或 `__file__` 来推断根目录**。
...
2. 环境变量  CODA_ROOT                             → source="env"
3. 从 CWD 向上查找含 .coda/ 的最近祖先目录         → source="marker"
...
实现位置：`coda/core/workspace.py`（Phase 1 任务 1.2，见 §8）。
...
- **CLI 入口**：`coda`（REPL）+ `coda exec "<prompt>"`（headless），暂无 TUI。
...
- **`.codaignore`**：fs/grep/glob 工具默认遵循（详见 §5.3）。
- **路径围栏**：所有 fs 工具强制 resolve 到 workspace_root，拒绝符号链接逃逸。
- **危险命令黑名单**：`rm -rf /`、`sudo`、`curl ... | sh`、`git push --force`、`:(){:|:&};:` 等。
- **`AGENTS.md` 自动加载**（项目根）。
- **Session 持久化**：SQLite event-sourcing，`coda resume [session_id]` 恢复。
- **结构化日志**（JSONL）+ Token/成本实时显示。
- **流式输出**：所有 LLM 响应流式渲染，Ctrl-C 可中止并保留已生成内容。
- **配置系统**：`<project>/.coda/config.yaml` + `~/.config/coda/config.toml` + 环境变量三级覆盖。
...
- **`coda init`**：在项目根生成 `.coda/config.yaml` 与 `AGENTS.md` 模板。
- **`coda undo`**：基于 git checkpoint 回退到上一次工具调用前状态。
...
文件位置：`<project_root>/.codaignore`，语法兼容 `.gitignore`。MVP 启动时按以下顺序合并 ignore 规则：
...
2. **项目 `.gitignore`**（自动尊重）。
3. **项目 `.codaignore`**（用户自定义补充）。
4. **用户级 `~/.config/coda/codaignore`**。

强制行为：
- 所有 `read_file` / `glob` / `grep` / `list_dir` 调用都先过 ignore 检查。
- 命中 ignore 的路径返回明确错误：`PathIgnoredError: <path> is ignored by .codaignore (rule: <rule>)`，让模型理解原因而不是默默失败。
...
- `coda session prune` 主动清理过期 session 与 checkpoint 引用
- 关键 checkpoint（如成功 PR 前）可显式 `coda checkpoint pin <session_id>` 升级为正式 tag
...
- 1-5 段：硬编码在 `coda/core/prompts/system.py` 中，跟随 release 版本演进；项目可覆盖部分段落（通过 `.coda/config.yaml` 的 `prompts.override`）。
...
```
1. 系统级（可选）  ~/.config/coda/AGENTS.md           # 全局个人偏好（如"用 type hints"）
...
- 不在其中定义工具、不定义 hook、不定义自动化（这些放 `.coda/config.yaml`）。
...
采用 Python 主流的 `src/` layout 单包结构。即使内部按子模块组织，对外仍是同一个 `coda` 包。这种结构 uv / hatch / setuptools 原生支持，IDE 跳转和 Aider / SWE-agent / OpenHands 一致。
...
- **错误恢复困难** [H × M]：自动 git checkpoint + `coda undo`；工具错误格式统一（给模型与给用户分两路渲染）。
...
- **工具执行超时**
  ...
  - 处理：kill 子进程树（`os.killpg` 或 `ProcessGroup`）→ 触发 `coda undo` 自动回滚到上一 checkpoint → 回填超时错误 + 部分输出（截断）让模型决定下一步。
...
- **上下文压缩导致信息丢失**
  ...
  - 处理：压缩前提取并固定保留"近期 tool_call 涉及的文件路径列表" + "用户最近一次明确指令"；压缩事件写 `SessionEvent`；终端 banner 提示"上下文已压缩，必要时 `coda resume` 切回非压缩 session"。
...
- **provider 限流 / 5xx**
  - 现象：HTTP 429 / 500 / 503。
  - 处理：LiteLLM router 自动 fallback 到下一家；fallback chain 全部失败时指数退避重试（最多 3 次，base 1s）；仍失败则保存 session 状态并让用户 `coda resume` 恢复。
...
- **Workspace 与用户预期不一致（在错误目录启动 / 误用 --root）**
  ...
  - 处理：(1) 审批 UX 在**每次首次写操作**前显式渲染 `<绝对路径> (workspace=<root>, source=<source>)`，让用户在批准前看到完整上下文；(2) 用户拒绝时提示 "如需切换工作区请退出后用 `coda --root <path>` 重启（或设置 `CODA_ROOT`）"；(3) 禁止运行时动态切换 root——这是不变量（见 §3.4.2），切换需重启进程。
...
- 错误恢复逻辑集中在 `coda/core/recovery.py` 模块（不升格为独立 Protocol，避免接口爆炸；如未来确实跨模块复用，再按 §11 原则 6 抽象）。
...
- **依赖膨胀** [M × M]：核心依赖 ≤ 15 个；可选功能走 `extras`（`pip install coda[tui,vector]`）。
...
- **1.2 模块骨架 + Protocol** — 产出：按 §6 目录创建空包，所有 §3.4 Protocol 落地，含 `coda/core/workspace.py`（`Workspace` 值对象 + `WorkspaceResolver` 实现 + 启动 banner 打印 root/source）｜ 验收：(a) `from coda.llm import LLMProvider` 等导入全部成功，mypy strict 通过；(b) Workspace 发现优先级 5 条全部有单测（用 `tmp_path` 构造 `.coda/` / `.git/` 标记目录，覆盖 flag / env / marker / cwd 四种 source）；(c) E2E 场景测试：在 CWD ≠ 项目根（如 `CWD=/tmp`）时启动 CLI，banner 打印的 root / source 与预期一致。
- **1.3 LLM Provider 层** — 产出：`coda/llm` 的 LiteLLM 实现 + 流式 + 重试 + cost tracker ｜ 验收：单测覆盖 ≥ 90%，能切换 Claude/OpenAI 两家而不改 caller 代码。
- **1.4 Agent Loop** — 产出：`coda/core` 的 Session / Context / Agent Loop（含 token 预算 + 80% 压缩 + `recovery.py` 错误恢复模块）｜ 验收：用 mock LLM 跑 50+ turn 不爆窗口；§7.5 八个错误恢复场景全部有单测覆盖。
...
- **2.4 Textual TUI** — 产出：任务面板 + 实时 diff + 流式日志 + 多 session 切换 ｜ 验收：`coda tui` 启动，与 REPL 共享 core。
...
- **3.6 安全加固** — 产出：prompt injection 测试集 + `.codaignore` 边界测试 + 依赖审计 CI ｜ 验收：覆盖 OWASP LLM Top10 相关项。
...
- **4.2 单文件分发** — PyInstaller / shiv 打包 + Homebrew tap + Scoop bucket ｜ 验收：`brew install coda` 可用。
...
- 2026-05-06 v0.2：与开发同学方案对照后修订。主要变更：
  ...
  - §5 改为 MoSCoW 三档；MVP 工具集从 8 个扩到 11 个（新增 git 三件套）；新增 §5.3 `.codaignore` 设计与默认列表；新增 §5.4 Git 安全网。
...
- 2026-05-06 v0.4：基于 [ARCHITECTURE_REVIEW_v0.3](reviews/ARCHITECTURE_REVIEW_v0.3.md) 修订，全部为跨节一致性回归与措辞精确化。主要变更：
  ...
  - §8 Phase 1 任务 1.3 / 1.4 / 1.6 中 `packages/llm` `packages/core` `packages/sandbox` 路径改为 `coda/llm` `coda/core` `coda/sandbox`，与 §6.1 一致（修 A2）。
  ...
  - §7.5 `RecoveryCoordinator` 降级为 `coda/core/recovery.py` 模块（不升格为独立 Protocol），与 §11 原则 6 一致；显式约定调用规范（修 B3）。
...
- 2026-05-06 v0.3：基于 [ARCHITECTURE_REVIEW_v0.2](reviews/ARCHITECTURE_REVIEW_v0.2.md) 修订。主要变更：
  - §6 重写为 `src/coda/` 扁平 layout，与 §12 First Step 一致；明确拒绝 `apps/+packages/` monorepo（修 A1）。
...
5. **Git 是安全网**。每次写操作前自动 checkpoint（stash / 临时分支），永远可 `coda undo`。
...
```bash
mkdir -p coda && cd coda
uv init --name coda --python 3.12
...
touch src/coda/__init__.py
for d in cli core llm tools sandbox memory obs; do touch "src/coda/$d/__init__.py"; done

mkdir -p .coda
cat > .coda/config.yaml <<'EOF'
...
> **关于模型名**：示例使用的是 LiteLLM 的命名约定 `<provider>/<model-id>`，且 `<model-id>` 推荐带日期后缀（如 `claude-sonnet-4-5-20250929`）以保证可复现。具体可用模型以 LiteLLM 官方 [model list](https://docs.litellm.ai/docs/providers) 为准；MVP 可在 `coda doctor` 命令中加 provider 连通性检查。

启动时 Workspace 发现优先级（对应 §3.4.2）：

```bash
# 优先级 1：CLI flag --root（推荐在非项目目录运行时显式指定）
coda --root ~/projects/foo "帮我优化 main.py"

# 优先级 2：环境变量 CODA_ROOT
export CODA_ROOT=~/projects/foo
coda "帮我优化 main.py"

# 优先级 3/4：自动向上查找 .coda/ 或 .git/（无需额外操作，在项目目录内 cd 即可）
cd ~/projects/foo
coda "帮我优化 main.py"
...
> **注意**：`scripts/prototype.py`（Phase 0 原型）同样支持 `--root` / `CODA_ROOT` / CWD 三档，已在 Phase 0 完成修复（见 §10 v0.5 changelog）。

...
§3.4.2:
    ...
    2. Env var     CODA_ROOT           → source="env"
    3. Ancestor with .coda/ directory  → source="marker"
...
class LocalWorkspaceResolver:
    ...
    def _discover(self, explicit: Path | None) -> tuple[Path, WorkspaceSource]:
        ...
        env_root = os.environ.get("CODA_ROOT")
        if env_root:
            return Path(env_root).expanduser().resolve(), "env"

        cwd = Path.cwd().resolve()

        # Priority 3: ancestor containing .coda/
        marker = self._find_ancestor_with(cwd, ".coda")
    ...
    def _collect_memory_paths(root: Path) -> list[Path]:
        candidates = [
            root / "AGENTS.md",
            Path.home() / ".config" / "coda" / "AGENTS.md",
        ]

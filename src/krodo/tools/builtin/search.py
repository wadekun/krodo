...
(_NOISE_DIRS) without requiring .krodoignore (that is deferred to M4).
...
from krodo.sandbox.path_filter import _NOISE_DIRS, filter_allowed_paths
    ...
    *,
    ...
    for path in sorted(base.rglob(file_pattern)):
        ...
        if any(part in _NOISE_DIRS for part in rel.parts):
            continue
        # Skip paths ignored by KrodoIgnore

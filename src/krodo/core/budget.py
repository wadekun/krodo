...
- Apply a per-model token ratio (Claude 1.1×, others 1.0×) configurable via
  the KRODO_TOKEN_RATIO environment variable.
...
# tiktoken's gpt-4o encoding predicts.  Configurable via KRODO_TOKEN_RATIO.
...
def _get_ratio(model: str) -> float:
    """Return the token ratio for *model*, honouring KRODO_TOKEN_RATIO env var."""
    env_ratio = os.environ.get("KRODO_TOKEN_RATIO")
    if env_ratio:
        try:

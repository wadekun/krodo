"""CodaGroup — custom Click/Typer Group that pre-routes subcommands.

PROBLEM
-------
The main Typer app declares both a positional ``prompt`` Argument (for
``coda "task"`` headless mode) and three sub-apps (``undo`` / ``resume`` /
...
``prompt`` and never reaches the subcommand router.  This causes:
  ...
  * ``coda resume --root /X``  → "No such command '--root'" error
  * ``coda --root /X resume``  → silent failure (LLM gets prompt="resume")
...
---------------------------------
When the user writes ``coda --root /X resume``, the group-level ``--root``
...
class CodaGroup(TyperGroup):
    """TyperGroup subclass that correctly routes Coda's named subcommands.
    ...
    def _propagate_defaults(self, ctx: click.Context, cmd_name: str) -> None:
        ...
        This allows ``coda --root /X resume`` to work correctly: the group

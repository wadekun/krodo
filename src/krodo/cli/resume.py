"""krodo resume — reload a previous session into an interactive REPL (M5.2).

Usage::

    krodo resume                   # resume the most recent session in cwd workspace
    krodo resume <session_id>      # resume a specific session by full or prefix ID
    krodo resume --list            # list the 10 most recent sessions
...
if TYPE_CHECKING:
    from krodo.cli.main import SessionComponents
    from krodo.core.types import Message
    from krodo.memory.store import SessionRow
...
def resume_command(
    ...
    *,
    ...
    from krodo.core.config import (  # noqa: PLC0415
        load_config,
        resolve_repo_map,
    ...
    # M10: repo-map resolution, same pattern.
    ...
    if resolved_id is None:
        if session_id:
            console.print(
                f"[red]No session matching '{session_id}' in "
                f"{workspace_root}/.krodo/sessions/[/red]"
            )
        else:
            console.print(
                f"[red]No sessions found in {workspace_root}/.krodo/sessions/. "
                "Did you run krodo in this workspace?[/red]"
            )
        raise typer.Exit(code=1)

    async def _entry() -> None:
        from krodo.cli.main import _build_session_components  # noqa: PLC0415
...
async def repl_session_cycle(
    ...
    """
    from krodo.cli.repl import run_repl  # noqa: PLC0415
...
def register_resume_app(app: typer.Typer) -> None:
    """Register ``krodo resume`` as a subcommand."""
    resume_sub = typer.Typer(
        name="resume",
        help="Resume a previous Krodo session.",
    ...
    def _resume(
        ...
        """Resume a previous Krodo session in the current workspace."""
        resume_command(

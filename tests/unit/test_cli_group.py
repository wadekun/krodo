"""Unit tests for KrodoGroup — the custom Click/Typer Group that pre-routes subcommands.
...
# ---------------------------------------------------------------------------
# Helpers: build a minimal KrodoGroup-backed app for testing
# ---------------------------------------------------------------------------


def _make_app() -> typer.Typer:
    """Minimal Typer app with KrodoGroup and the same option layout as `krodo`."""
    app = typer.Typer(
        name="krodo",
        cls=KrodoGroup,
...
def _get_group(app: typer.Typer) -> KrodoGroup:
    """Extract the underlying Click Group from a Typer app."""
    click_app = typer.main.get_command(app)
    assert isinstance(click_app, KrodoGroup)
    return click_app
...
class TestFindSubcommandSplit:
    """Tests for KrodoGroup._find_subcommand_split."""

    @pytest.fixture()
    def grp(self) -> KrodoGroup:
        app = _make_app()
        return _get_group(app)

    @pytest.fixture()
    def ctx(self, grp: KrodoGroup) -> click.Context:
        return click.Context(grp)

    def test_subcommand_first_token(self, grp: KrodoGroup, ctx: click.Context) -> None:
    ...
    def test_undo_subcommand(self, grp: KrodoGroup, ctx: click.Context) -> None:
        """undo is also a registered subcommand."""
        result = grp._find_subcommand_split(ctx, ["undo"])  # noqa: SLF001
        assert result == (0, "undo")

    def test_doctor_subcommand(self, grp: KrodoGroup, ctx: click.Context) -> None:
...
class TestPropagateDefaults:
    """Tests for KrodoGroup._propagate_defaults."""

    @pytest.fixture()
    def grp(self) -> KrodoGroup:
        return _get_group(_make_app())

    def test_propagates_root(self, grp: KrodoGroup) -> None:
    ...
    def test_does_not_propagate_none(self, grp: KrodoGroup) -> None:
        ctx = click.Context(grp)
    ...
    def test_does_not_override_existing_default_map(self, grp: KrodoGroup) -> None:
        ctx = click.Context(grp)
    ...
    def test_does_not_propagate_prompt(self, grp: KrodoGroup) -> None:
        ctx = click.Context(grp)
        ctx.params = {"prompt": "some task", "root": "/X"}
        grp._propagate_defaults(ctx, "resume")  # noqa: SLF001
        assert "prompt" not in (ctx.default_map or {}).get("resume", {})

    def test_multiple_shared_keys(self, grp: KrodoGroup) -> None:
        ctx = click.Context(grp)
...
class TestParseArgs:
    """Integration-style tests verifying ctx state after KrodoGroup.parse_args."""

    @pytest.fixture()
    def grp(self) -> KrodoGroup:
        return _get_group(_make_app())

    def _make_ctx(self, grp: KrodoGroup) -> click.Context:
        ctx = click.Context(grp, resilient_parsing=False, info_name="krodo")
        ctx.ensure_object(dict)
        return ctx

    def test_subcommand_token_sets_protected_args(self, grp: KrodoGroup) -> None:
    ...
    def test_subcommand_with_options_after(self, grp: KrodoGroup) -> None:
    ...
    def test_group_option_before_subcommand(self, grp: KrodoGroup) -> None:
    ...
    def test_group_option_propagated_to_default_map(self, grp: KrodoGroup) -> None:
    ...
    def test_plain_prompt_not_routed(self, grp: KrodoGroup) -> None:
    ...
    def test_empty_args_not_routed(self, grp: KrodoGroup) -> None:
    ...
    def test_subcommand_with_session_id(self, grp: KrodoGroup) -> None:
    ...
    def test_undo_subcommand_routed(self, grp: KrodoGroup) -> None:
    ...
    def test_doctor_subcommand_routed(self, grp: KrodoGroup) -> None:

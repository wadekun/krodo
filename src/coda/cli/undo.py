...
def undo_command(
    ...
    *,
    ...
    if mgr.git_root is None:
        console.print(
            "[red]This session was not run inside a git repository. coda undo requires git.[/red]"
        )
        raise typer.Exit(code=1)
...
def register_undo_app(app: typer.Typer) -> None:
    """Register the ``coda undo`` subcommand using add_typer so the main
    command remains invokable without a subcommand prefix."""
    undo_sub = typer.Typer(
        name="undo",
        help="Restore files to the previous checkpoint created by Coda.",

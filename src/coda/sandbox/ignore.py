"""CodaIgnore — 4-tier path filtering for all fs tools (architecture.md §5.3).
...
Merges ignore rules from four sources, in increasing specificity order:
    ...
    3. Project .codaignore  (<workspace_root>/.codaignore)
    4. User-level codaignore (~/.config/coda/codaignore)
...
# ---------------------------------------------------------------------------
# Hard-coded defaults (§5.3 §1 — cannot be disabled by any .codaignore)
# ---------------------------------------------------------------------------
...
_SOURCE_CODAIGNORE = ".codaignore"
_SOURCE_USER = "user-codaignore"
...
class CodaIgnore:
    ...
    def __init__(
        ...
        *,
        ...
        # Tier 3: .codaignore
        self._codaignore = self._load_spec(workspace_root / ".codaignore")

        # Tier 4: user-level
        user_path = Path.home() / ".config" / "coda" / "codaignore"
    ...
    def match(self, path: Path, *, relative_to: Path | None = None) -> MatchResult:
        ...
        *path* may be absolute or relative; it is normalised to be relative to
        ...
        # Check tiers in priority order: hardcoded → gitignore → codaignore → user
        for spec, source in (
            (self._hardcoded, _SOURCE_HARDCODED),
            (self._gitignore, _SOURCE_GITIGNORE),
            (self._codaignore, _SOURCE_CODAIGNORE),
    ...
    @classmethod
    def from_workspace(cls, workspace: Workspace) -> CodaIgnore:
        """Create a CodaIgnore from a Workspace value object."""

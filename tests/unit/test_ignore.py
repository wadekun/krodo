"""Tests for CodaIgnore — 4-tier path filtering (M4 PR1)."""
...
from coda.sandbox.ignore import CodaIgnore, PathIgnoredError
...
def make_ignore(tmp: Path, *, gitignore: str = "", codaignore: str = "") -> CodaIgnore:
    """Create a CodaIgnore rooted at *tmp* with optional ignore file content."""
    if gitignore:
        (tmp / ".gitignore").write_text(gitignore)
    if codaignore:
        (tmp / ".codaignore").write_text(codaignore)
...
class TestGitignore:
    ...
    def test_missing_gitignore_ok(self, tmp_path: Path) -> None:
        ig = CodaIgnore(tmp_path)  # no .gitignore
...
# ---------------------------------------------------------------------------
# Tier 3 — project .codaignore
# ---------------------------------------------------------------------------


class TestCodaignore:
    def test_codaignore_pattern_respected(self, tmp_path: Path) -> None:
        ig = make_ignore(tmp_path, codaignore="secrets/\n")
        assert ig.is_ignored(tmp_path / "secrets" / "token.txt")

    def test_codaignore_overrides_below_tiers(self, tmp_path: Path) -> None:
        # Not in .gitignore, but in .codaignore
        ig = make_ignore(tmp_path, gitignore="*.log\n", codaignore="config.yaml\n")
        assert ig.is_ignored(tmp_path / "config.yaml")
        assert ig.is_ignored(tmp_path / "app.log")  # gitignore still active

    def test_codaignore_source_reported(self, tmp_path: Path) -> None:
        ig = make_ignore(tmp_path, codaignore="internal/\n")
        result = ig.match(tmp_path / "internal" / "data.csv")
        assert result.is_ignored
        assert result.source == ".codaignore"

    def test_missing_codaignore_ok(self, tmp_path: Path) -> None:
        ig = CodaIgnore(tmp_path)  # no .codaignore
        assert not ig.is_ignored(tmp_path / "src" / "main.py")
...
class TestUserLevelIgnore:
    def test_user_level_pattern_respected(
        ...
        user_cfg = tmp_path / ".config" / "coda"
        user_cfg.mkdir(parents=True)
        (user_cfg / "codaignore").write_text("*.secret\n")
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        ig = CodaIgnore(tmp_path / "project")
        ig._user = CodaIgnore._load_spec(user_cfg / "codaignore")  # type: ignore[attr-defined]
        assert ig.is_ignored(tmp_path / "project" / "api.secret")

    def test_missing_user_config_ok(self, tmp_path: Path) -> None:
        ig = CodaIgnore(tmp_path)  # user cfg doesn't exist
        assert not ig.is_ignored(tmp_path / "app.py")
...
class TestEdgeCases:
    ...
    def test_comment_lines_in_codaignore(self, tmp_path: Path) -> None:
        (tmp_path / ".codaignore").write_text("# This is a comment\n*.log\n")
        ig = CodaIgnore(tmp_path)
    ...
    def test_from_workspace(self, tmp_path: Path) -> None:
        ...
        ws = MagicMock()
        ws.root = tmp_path
        ig = CodaIgnore.from_workspace(ws)
        assert ig.is_ignored(tmp_path / ".env")

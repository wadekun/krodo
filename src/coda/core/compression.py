...
Two strategies, selected via the CODA_COMPRESS environment variable:

  CODA_COMPRESS=llm (default)
...
def make_compressor(
    ...
    2. ``CODA_COMPRESS`` environment variable.
    ...
    effective = (
        strategy
        or os.environ.get("CODA_COMPRESS")
    ...
    if effective == "llm":
        if provider is None:
            raise ValueError(
                ...
                "Set CODA_COMPRESS=algorithmic or pass a provider."
            )
